  ...
  <div class="col-sm-12 stretch-card" style="position: relative;">
    <div class="card"> 
       <div class="card-body">
         ...
         <br/>
         <form class="forms-sample" id="apriForm" data-loader-file="/image/gif/cube.gif" data-url-post="/human-resource/master-data/business-unit/act/edit">
          {{ csrf_field() }}
            <input type="hidden" name="BUSINESS_UNIT_ID" value="{{ $data->BUSINESS_UNIT_ID }}">
            <div class="form-group">
               <label for="COMPANY_NAME">Code</label>
               <input type="text" class="form-control" id="BUSINESS_UNIT_CODE" name="BUSINESS_UNIT_CODE" placeholder="Enter Business Unit Code..." value="{{$data->BUSINESS_UNIT_CODE}}" required>
            </div>
            <div class="form-group">
               <label for="COMPANY_NAME">Name</label>
               <input type="text" class="form-control" id="BUSINESS_UNIT_NAME" name="BUSINESS_UNIT_NAME" placeholder="Enter Business Unit Name..." value="{{$data->BUSINESS_UNIT_NAME}}" required>
            </div>
            <div class="form-group">
              <label for="BUSINESS_UNIT_DESCRIPTION">Description</label>
              <textarea class="form-control" id="BUSINESS_UNIT_DESCRIPTION" name="BUSINESS_UNIT_DESCRIPTION" rows="4"  placeholder="Enter Business Unit Description..." required>{{$data->BUSINESS_UNIT_DESCRIPTION}}</textarea>
            </div>
            <div class="form-group">
          ...
          if (data.success) {
            if (data.msg) {
                Swal.fire({
                  ...
                  window.location.href="/human-resource/master-data/business-unit/edit/{{ $data->BUSINESS_UNIT_ID }}";
                });
