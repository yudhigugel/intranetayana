...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb breadcrumb-custom bg-inverse-primary">
    ...
    <li aria-current="page" class="breadcrumb-item active"><span>PNL</span></li></ol>
</nav>
           ...
           <div class="form-group mb-3">
              <div class="row">
                <div class="title-header col-12 text-center" style="position: relative;display: flex;flex-direction: column;justify-content: center;align-items: center;">
                  ...
                  <div>
                    <form class="form" method="POST" action="{{ route('sap.report.pnl') }}" id="pnl-form">
                      {{ csrf_field() }}
                      <div>
                      ...
                      <div>
                        <h3 class="text-black"> Profit & Loss Report </h3>
                      </div>
                      <div>
                  ...
                  <div style="position: absolute; top:0; right:0">
                    <div class="d-inline-block">
                      ...
                      <div class="d-inline-block mx-1">
                        <button class="btn btn-secondary btn-sync btn-sm" disabled><i class="fa fa-refresh"></i>&nbsp;&nbsp;Sync</button>
                      </div>
                    </div>
           ...
           <div class="main-wrapper" id="tab-wrapper">
              <div class="row align-items-end">
                <div class="col-6 mb-3 text-left">
                  <div>
                    <h5>
                      {!! isset($data['VALUE_PNL']) ? '<i class="fa fa-check text-success"></i>&nbsp;'.(count($data['VALUE_PNL']) + 1)." P&L Templates Available" : 'No PNL Template Available' !!}
                    </h5>
                  </div>
                </div>

                <div class="col-6 mb-3 text-right">
                  <button class="btn btn-primary btn-sm btn-secondary btn-download-all"><i class="mdi mdi-download"></i>&nbsp; Export All</button>
                </div>
              </div>
              <div class="row">
                <div class="col-12">
                  ...
                  <ul class="nav nav-tabs" role="tablist" style="flex-wrap: nowrap;white-space: nowrap;overflow-x: scroll;overflow-y: hidden;">
                    <li class="nav-item">
                      <a class="nav-link active" id="summary-all-tab" data-toggle="tab" href="#summary-all" role="tab" aria-selected="true">Summary P&L (All)
                        <span class="export-btn" style="display: inline-block !important;">&nbsp; | &nbsp;<i class="mdi mdi-arrow-down-bold-circle-outline btn-download-sum-all download-excel text-primary" data-title="Summary P&L (All)" data-key="summary-all"></i></span>
                      </a>
                    </li>
                    ...
                    @if(isset($data['VALUE_PNL']) && count($data['VALUE_PNL']))
                      @foreach($data['VALUE_PNL'] as $key_report => $data_pnl)
                        <li class="nav-item">
                          <a class="nav-link" id="{{ $key_report }}-tab" data-toggle="tab" href="#{{ $key_report }}" role="tab" aria-selected="true">
                  ...
                  <div class="tab-content">
                    <div class="tab-pane fade active show" id="summary-all" role="tabpanel" aria-labelledby="summary-all-tab">
                      <div class="table-wrapper" style="overflow-x: hidden;">
                        <div class="text-center pt-3 pb-4" style="margin-right: 20px">
                          <h3 style="border-bottom: 1px solid #d2d2d2; line-height:0.1em;"><span style="background:#fff; padding:0 10px;">SUMMARY P&L (ALL)</span></h3>
                        </div>
                        @includeIf('pages.report.finance.pnl_part.summary_all_new', ['data' => isset($data) ? $data : [] ])
                      </div>
                    </div>
                    @if(isset($data['VALUE_PNL']) && count($data['VALUE_PNL']))
                      @foreach($data['VALUE_PNL'] as $key_report_tab => $data_pnl_tab)
                        @php
                          $pages = 'pages.report.finance.pnl_part.'.$key_report_tab;
                        @endphp
                        <div class="tab-pane fade" id="{{ $key_report_tab }}" role="tabpanel" aria-labelledby="{{ $key_report_tab }}-tab">
                          <div class="table-wrapper" style="overflow-x: hidden;">
                            <div class="text-center pt-3 pb-4" style="margin-right: 20px">
...
<script>
   ...
   $(document).ready(function() {
        ...
        $(document).on('click', '.download-excel', function(e){
          if(!export_all_status){
            var title = $(this).data('title') || 'PNL Report';
            var key = $(this).data('key') || '#none';
            var mS = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Period 13', 'Period 14', 'Period 15', 'Period 16'];
        ...
        $(document).on('click', '.btn-download-all', function(){
          ...
          Swal.fire({
            title: 'Export All Confirmation',
            text: `This operation will export all P&L data to Excel workbook`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            confirmButtonColor: '#3085d6',
            ...
            if (result.isConfirmed) {
              ...
              setTimeout(function(){
                try {
                  ...
                  $($this_btn).html('<i class="fa fa-spin fa-spinner"></i>&nbsp; Exporting');
                  $.ajax({
                    url : '/daily_report/type/pnl', 
                    method : 'GET',
                    xhrFields: {
                        responseType: 'blob'
                    },
                    data : {'company': "{{ isset($data['COMPANY_NAME']) ? $data['COMPANY_NAME'] : 'KMS1' }}", 'month': "{{ isset($data['MONTH']) ? $data['MONTH'] : '' }}", 'year': "{{ isset($data['YEAR']) ? $data['YEAR'] : '' }}"},
                    ...
                    error: function(e){
                      Swal.fire('Export P&L', 'Something went wrong when trying to export data, please try again later', 'error');
                    },
                    complete : function(){
                      $('.btn').prop('disabled', false);
                      $($this_btn).html('<i class="mdi mdi-download"></i>&nbsp; Export All');
                      export_all_status = false;
                    }
                  });
                } catch(error){
                  Swal.fire('Export P&L', 'Something went wrong when trying to export data, please try again later', 'error');
          ...
          // var url = "{{URL::route('sap.report.pnl.export')}}?company=KMS1&year=2022&month=6";
          // window.location = url;
        });
    });
  </script>
