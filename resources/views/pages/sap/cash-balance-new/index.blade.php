	    ...
	    <div class="col-sm-12 stretch-card" style="position: relative;">
  	     <div class="card">
  				<div id="reactiveListener">
  					<cash-balance type_balance_new="true" parent_need_password="{{ isset($data['IS_NEED_PASSWORD']) ? $data['IS_NEED_PASSWORD'] ? 1 : 0 : 1 }}" year_lookup="{{ isset($data['FISCAL_YEAR']) ? $data['FISCAL_YEAR'] : date('Y') }}" last_update_exchange="{{ date('d F Y') }}" date_start_contract="{{ date('m/d/Y',strtotime($data['date_start'])) }}" date_end_contract="{{ date('m/d/Y',strtotime($data['date_end'])) }}" template-based-on-role="{{ isset($data['IS_NEED_PASSWORD']) ? $data['IS_NEED_PASSWORD'] ? '' : $template : '' }}"></cash-balance>
  				</div>
  			</div>
    	</div>
	</div>
