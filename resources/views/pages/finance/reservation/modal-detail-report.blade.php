...
<form id="modalDetailAjax" method="POST" id="formChangeRequest" enctype="multipart/form-data" data-url-post="{{url('finance/add-reservation/request/update')}}" data-loader-file="{{ url('/image/gif/cube.gif') }}">
    ...
    <div class="form-group">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <input type="text" name="CreatedDate" value="{{ date('d F Y', strtotime(isset($data['data_form']->INSERT_DATE) ? $data['data_form']->INSERT_DATE : 'Ymd' )) }}" class="form-control" readonly/>
            </div>
            <div class="col-md-6">
    ...
    <div class="form-group">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-4">
                ...
                <select required class="select2 select-decorated form-control" disabled name="MovementType" id="movementTypeShow" style="width: 100%; padding: 0.8rem 1.375rem">
            ...
            <div class="col-md-4">
                ...
                <input type="text" {{ $editable }} class="form-control" style="padding: 0.78rem 1.375rem" name="RequestNote" value="{{ isset($data['data_json']->RequestNote) ? $data['data_json']->RequestNote : '' }}" placeholder="Insert reservation notes here.."/>
            </div>
            <div class="col-md-4">
            ...
            <div class="row">
                <div class="col-md-2">
                    ...
                    <div style="position: relative;">
                        <select {{ $editable }} class="form-control select2 select-decorated" name="rsvReceivingPlant" id="receiving_plant_new" style="width: 100%">
                ...
                <div class="col-md-10">
                    ...
                    <select {{ $editable }} class="form-control select2 select-decorated" name="rsvReceivingPlantSLOC" id="receiving_plant_sloc_new" style="width: 100%">
                        ...
                        @if(isset($data['s_loc_receiving']) && count($data['s_loc_receiving']) > 0)
                            @foreach($data['s_loc_receiving'] as $key => $val)
                                <option @if(isset($data['data_json']->rsvReceivingPlantSLOC) && $data['data_json']->rsvReceivingPlantSLOC == $val['STORAGE_LOCATION']) selected @endif value="{{ isset($val['STORAGE_LOCATION']) ? $val['STORAGE_LOCATION'] : '' }}">{{ isset($val['STORAGE_LOCATION']) ? $val['STORAGE_LOCATION'].' - ' : '' }} {{ isset($val['STORAGE_LOCATION_DESC']) ? $val['STORAGE_LOCATION_DESC'] : 'UNKNOWN STORAGE LOCATION' }}</option>
                            @endforeach
                        @endif
                    </select>
                    ...
                    <tbody>
                        @if(isset($data['data_json']->rsvItem) && count($data['data_json']->rsvItem) > 0)
                            ...
                            <tr class="rowToClone">
                                ...
                                <td style="max-width: 1px">
                                   <div style="position: relative;">
                                       <select {{ $editable }} class="form-control select2 select-decorated" name="rsvOriginPlant[]" id="plantNew{{ $loop->iteration }}" style="width: 100%">
                                            <option value="" selected>---- Choose Plant ----</option>
                                            @if(isset($data['plant_list']) && count($data['plant_list']) > 0)
                                                @foreach($data['plant_list'] as $key => $val)
                                ...
                                <td style="max-width: 1px">
                                   <select {{ $editable }} class="form-control select2 select-decorated" name="rsvSLOC[]" id="sloc{{ $loop->iteration }}" style="width: 100%">
                                ...
                                <td style="max-width: 1px">
                                    <select {{ $editable }} class="form-control select2 select-decorated-material" name="rsvMaterials[]" id="materials{{ $loop->iteration }}" style="width: 100%">
                                ...
                                <td style="max-width: 1px; position: relative;">
                                    <input {{ $editable }} type="text" oninput="qtyInput(this)" class="form-control text-center" name="rsvQuantity[]" id="rsvQty" value="{{ isset($data['data_json']->rsvQuantity[$index]) ? $data['data_json']->rsvQuantity[$index] : 1 }}">
                                ...
                                @if(isset($data['status_approval']) && $data['status_approval'] == false)
                                    <td>
                                        <div class="btn-group" style="min-width:80px">
                                            ...
                                            <button type="button" class="btn btn-danger btn-sm px-1 btn-del" onclick="deleteBaris('reqFormWithPlantDetail')">-</button>
                                            @else
                                            <button type="button" class="btn btn-danger btn-sm px-1 btn-del" onclick="deleteBaris('reqFormWithPlantDetail')">-</button>
        ...
        @if(isset($data['data_json']->rsvReceivingSLOC) && $data['data_json']->rsvReceivingSLOC)
            <div class="form-group" id="rsvSlocContainer">
                <div class="row">
                    <div class="col-md-12">
                        <label>Receiving SLOC <span class="red">*</span></label>
                        <select {{ $editable }} class="form-control select2 select-decorated" name="rsvReceivingSLOC" id="receiving_sloc" style="width: 100%">
                            <option value="" selected>---- Choose Sloc ----</option>
                            @if(isset($data['s_loc']) && count($data['s_loc']) > 0)
                                @foreach($data['s_loc'] as $key => $val)
                                    <option @if($data['data_json']->rsvReceivingSLOC == $val['STORAGE_LOCATION']. ' -  '.$val['STORAGE_LOCATION_DESC']) selected @endif value="{{ isset($val['STORAGE_LOCATION']) ? $val['STORAGE_LOCATION'] : '' }}">{{ isset($val['STORAGE_LOCATION']) ? $val['STORAGE_LOCATION'].' - ' : '' }} {{ isset($val['STORAGE_LOCATION_DESC']) ? $val['STORAGE_LOCATION_DESC'] : 'UNKNOWN STIRANGE LOCATION' }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
            ...
            <div class="form-group" id="rsvCostCenterContainer">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <select {{ $editable }} class="form-control select2 select-decorated" name="rsvCostCenterExpense" id="cost_center_expense" style="width: 100%">
                            ...
                            <tr class="rowToClone">
                                ...
                                @if(isset($data['status_approval']) && $data['status_approval'] == false)
                                    <td>
                                        <div class="btn-group" style="min-width:80px">
                                            @if(isset($data['status_approval']) && $data['status_approval'] == false && isset($data['action']) && strtolower($data['action']) == 'approve')
                                            <button type="button" class="btn btn-danger btn-sm px-1 btn-del" onclick="deleteBaris('reqFormDetail')">Remove</button>
                                            @else
                                            <button type="button" class="btn btn-danger btn-sm px-1 btn-del" onclick="deleteBaris('reqFormDetail')">-</button>
