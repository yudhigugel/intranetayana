			...
			<div class="modal-body" id="bodymodalRequest" style="overflow: hidden">
                <form method="POST" id="formRequest" enctype="multipart/form-data" data-url-post="{{url('finance/cash-advance/request/save')}}" data-loader-file="{{ url('/image/gif/cube.gif') }}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="currency[]" id="currency" class="form-control" style="min-width:100px;padding: 0.75rem 1.375rem">
                                    <option value="IDR" selected>IDR</option>
                                    <option value="USD">USD</option>
                                </select>
                            </div>
                            <div class="col-md-6 required-amount" hidden>
                                <label><span class="text-danger">*</span> Request's total amount >= 1 Million requires PR Number</label>
                                <input type="text" value="" id="required-amount" name="Requested_PR_Number" placeholder="Input PR Number Here" class="form-control" readonly/>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="portlet-body table-both-scroll mb-3">
                            <table class="table table-striped table-bordered table-hover smallfont" id="reqForm">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="thead-apri" colspan="3"></th>
                                ...
                                <tbody>
                                    <tr id="rowToClone">
                                        ...
                                        <td>
                                            <input required type="text" value="1" class="form-control td-apri" style="min-width:75px;" name="quantity[]" id="quantity" onkeyup="CalculateTotal(this, 'reqForm');">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control td-apri" name="harga_parse[]" id="harga_parse" maxlength="12" onkeypress="return (event.charCode >= 48 &amp;&amp; event.charCode <= 57) || event.charCode==0 || event.charCode==46 || event.charCode==17 || event.charCode==86 || event.charCode==67" onkeyup="keyUpCurrency(this.value, this.id); CalculateTotal(this, 'reqForm');" placeholder="0.00" value="0">
                                        </td>
                                        <td>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input type="text" class="form-control" placeholder="0.00" readonly="" id="grandTotalText">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" id="bodyModalHistory">
                <table class="table table-bordered datatable requestHistoryList" id="requestHistoryList">
                    <thead>
                        <tr>
                            ...
                            <th style="max-width:100px;">Reason</th>
                        </tr>
                    </thead>
                ...
                <div id="settlement-approval-history-wrapper" class="pt-3" hidden>
                    <div class="mb-2">
                        <h4 class="mb-1">Additional Approval (Settlement)</h4>
                        <small class="text-muted">* This approval appears if only there is an extra amount than requested on returning backups</small>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function addRow(tableID) {
        ...
        if(rowCount<10000){
            ...
            for(var i=0; i<colCount; i++) {
                ...
                try {
                    if(table.rows[1].cells[i].classList.contains('not-visible')){
                        newcell.className = 'not-visible';
                    }
                } catch(error){}
    ...
    function cloneRow(tableID) {
        ...
        document.getElementById('grandTotalText').value = grandTotal;
    ...
    function CalculateTotal(idTag, tableID){
        setTimeout(function(){
            var rowId = idTag.id;
            ...
            console.log(idrow);
            quantity = parseFloat(document.getElementById('quantity'+idrow).value ? document.getElementById('quantity'+idrow).value : 1);
            ...
            try {
                if(grandTotal > 1000000){
                    $('.required-amount').prop('hidden', false);
                    // $('.required-amount').show();
                    document.getElementById('required-amount').removeAttribute('readonly');
                    document.getElementById('required-amount').setAttribute('required', true);
                }
                else {
                    $('.required-amount').prop('hidden', true);
                    // $('.required-amount').hide();
                    document.getElementById('required-amount').setAttribute('readonly', true);
                    document.getElementById('required-amount').setAttribute('required', false);
                }
            } catch(error){
                console.log('error validate grandTotal');
            }
            document.getElementById('grandTotal').value = grandTotal;
            grandTotal = grandTotal.toLocaleString('en-US', {minimumFractionDigits: 2});
            document.getElementById('grandTotalText').value = grandTotal;
    ...
    $(document).ready( function () {
        ...
        // addRow(this.getAttribute('data-tableID') || 'reqForm');
        // });
        // $(document).on('click','.btn-deleteBaris', function(){
        //     deleteBaris(this.getAttribute('data-tableID') || 'reqForm');
        // });
            ...
            "columns": [
                ...
                { "data": "STATUS_APPROVAL",
                    "render" : function (id, type, full, meta){
                        ...
                        if(id=="APPROVED" || id=="REQUESTED" || id=="" || id=="WAITING FOR APPROVAL"){
                            ...
                            if(typeof full.EXTENTED_CA != 'undefined' && full.EXTENTED_CA){
                                if(typeof full.EXTENTED_CA.STATUS_APPROVAL != 'undefined'){
                                    var status = full.EXTENTED_CA.STATUS_APPROVAL;
                                    let ext_number = (typeof full.EXTENTED_CA.UID != 'undefined') ? full.EXTENTED_CA.UID : 0;
                                    if(full.EXTENTED_CA.STATUS_APPROVAL == null || full.EXTENTED_CA.STATUS_APPROVAL == "null"){ status = "REQUESTED"; }
                                    if(full.EXTENTED_CA.STATUS_APPROVAL == "REQUESTED" || full.EXTENTED_CA.STATUS_APPROVAL=="APPROVED"){ status = "WAITING FOR APPROVAL"}

                                    if(status=="APPROVED" || status=="REQUESTED" || status=="" || status=="WAITING FOR APPROVAL"){
                                        return '<a href="#" data-toggle="modal" data-target="#modalHistory" onclick="getHistoryDetail(\''+full.UID+'\')" style="font-weight:bold;">'+status+'</a>';
                                    }
                                    else if(status=='REJECTED'){
                                        return '<a href="#" data-toggle="modal" data-target="#modalHistory" onclick="getHistoryDetail(\''+full.UID+'\')" style="font-weight:bold;color:red;">'+status+'</a>';
                                    }
                                } else {
                                    id = 'ON PROCESS';
                                    return '<a href="#" data-toggle="modal" data-target="#modalHistory" onclick="getHistoryDetail(\''+full.UID+'\')" style="font-weight:bold;color:#ff9400;">'+id+'</a>';
                                }
                            }
                ...
                { "data": "LAST_APPROVAL_NAME",
                    "render": function (id, type, full, meta)
                    {
                        $_name = id ? id : '-';
                        if(typeof full.EXTENTED_CA != 'undefined' && full.EXTENTED_CA){
                            if(typeof full.EXTENTED_CA.LAST_APPROVAL_NAME != 'undefined' && full.EXTENTED_CA.LAST_APPROVAL_NAME)
                                $_name = full.EXTENTED_CA.LAST_APPROVAL_NAME;
                        }
                        return $_name;
                    },
                    "className" : 'text-left'
    ...
    function getFormDetail(id){
        ...
        $.get("{{url('finance/cash-advance/modal-detail')}}", { id : id, segment : segment}, function( data ) {
    ...
    function getHistoryDetail(id){
        ...
        $('#historyListDT').html("");
        $('#settlement-approval-history-wrapper').html(`
            <div class="mb-2">
                <h4 class="mb-1">Additional Approval (Settlement)</h4>
                <small class="text-muted">* This approval appears if only there is an extra amount than requested on returning backups</small>
            </div>
        `);
        $('#settlement-approval-history-wrapper').prop('hidden', true);
            ...
            success: function(responseSIA){
                ...
                if(responseSIA.code == "200"){
                    if(responseSIA.data.length > 0)
                    {
                        ...
                        if(typeof responseSIA.data_settlement != 'undefined' && responseSIA.data_settlement.length){
                            var tr_settle = '';
                            for (i = 0; i < responseSIA.data_settlement.length; i++) {
                                if(changeNull(responseSIA.data_settlement[i].APPROVAL_DATE) == ""){
                                    var APPROVAL_DATE = "";
                                }
                                else{
                                    var APPROVAL_DATE = moment(responseSIA.data_settlement[i].APPROVAL_DATE).format("DD/MM/YYYY - HH:mm");
                                }
                                tr_settle += '<tr>';
                                tr_settle += '<td>'+changeNull(responseSIA.data_settlement[i].LEVEL_APPROVAL)+'</td>';
                                tr_settle += '<td>'+changeNull(responseSIA.data_settlement[i].TYPE_DESC)+'</td>';
                                tr_settle += '<td>'+changeNull(responseSIA.data_settlement[i].EMPLOYEE_NAME)+'</td>';
                                tr_settle += '<td>'+changeNull(responseSIA.data_settlement[i].STATUS_APPROVAL)+'</td>';
                                tr_settle += '<td>'+APPROVAL_DATE+'</td>';
                                tr_settle += '<td>'+changeNull(responseSIA.data_settlement[i].REASON)+'</td>';
                                tr_settle += '</tr>';
                            }

                            var table =`
                            <table class="table table-bordered datatable requestHistoryListSettlement" id="requestHistoryListSettlement">
                                <thead>
                                    <tr>
                                        <th style="min-width:90px;">Level Approval</th>
                                        <th style="min-width:90px;">Level Desc</th>
                                ...
                                <tbody id="historyListSettlementDT">
                                    ${tr_settle}
                                </tbody>
                            </table>
                            `;

                            $('#settlement-approval-history-wrapper').append(table);
                            $('#settlement-approval-history-wrapper').prop('hidden', false);
                            $('#requestHistoryListSettlement').DataTable();
                        }
                    }
                }else{
