...
@section('title', 'Marketlist Template - Add')
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb breadcrumb-custom bg-inverse-primary">
    ...
    <li aria-current="page" class="breadcrumb-item active"><span>Add Template</span></li></ol>
</nav>
  ...
  <div class="col-8 stretch-card" style="position: relative;">
    ...
    <div class="card">
      <div class="card-body main-wrapper" style="overflow-x: auto;overflow-y: hidden;">
        ...
        <h2 class="card-title"><i class="mdi mdi-plus"></i> Create Template</h2>
        <form method="POST" action="{{ url('/finance/purchase-requisition-marketlist/master-template/create') }}" id="form-marketlist">
          {{ csrf_field() }}
          <div class="form-group">
            <div class="row mb-3">
              <div class="col-12">
                ...
                <select class="select2 form-control" name="plant" id="plant-select" required style="width: 100%">
                  ...
                  @if(isset($data['plant_user']) && count($data['plant_user']))
                    @if(count($data['plant_user']) == 1)
                      ...
                      <option value="{{ $key_plant }}" selected default>{{ $val }}</option>
                      @endforeach
                    @else
                      @foreach($data['plant_user'] as $key_plant => $val)
                      <option value="{{ $key_plant }}">{{ $val }}</option>
                      @endforeach
                    @endif
                  @endif
                </select>
          ...
          <div class="form-group">
            <div class="row mb-3">
              <div class="col-12">
                ...
                <input type="text" name="template_name" id="template_name" required class="form-control" placeholder="Input Template Name" value="{{ old('template_name') }}">
                @error('template_name')
                <small class="text-danger mb-2 mt-1 d-block">
                  <i class="mdi mdi-alert-circle"></i>&nbsp;&nbsp;{{ $message }}
                </small>
              ...
              <div class="col-12">
                <div class="row align-items-end">
                  ...
                  <div class="col-5 text-right mb-2">
                    <button type="button" data-toggle="modal" data-target="#modalMaterial" class="btn btn-primary text-white btn-sm"><i class="fa fa-plus"></i>&nbsp; Add New / Edit Existing</button>
                  </div>
                </div>
                <div class="portlet-body table-both-scroll mb-4" style="overflow: auto; position: relative;">
                  <table class="table table-bordered" id="detailForm">
                    <thead>
                    ...
                    <tbody>
                        <tr>
                            <td colspan="5" class="text-center">No Material Selected</td>
                        </tr>
                    </tbody>                                
                  </table> 
                </div>
          ...
          <div class="form-group">
            <div class="form-group text-left d-flex">
              <div class="pr-2">
                <button type="button" class="btn btn-primary btn-submit" disabled><i class="mdi mdi-check"></i> Submit</button>
              </div>
              <div class="pr-2">
    ...
    <div class="modal-content">
      <div class="modal-header">
          <div>
            ...
            <div>
              <small class="text-muted">* Make sure all items are in the same purchasing group</small>
            </div>
          </div>
        ...
        <div class="form-group item-container">
          ...
          <div class="row portlet-body table-both-scroll mb-3" style="margin-bottom: 15rem !important;">
            <form class="form-horizontal" method="get" id='form-add-material' action='' style="width: 100%">
              <div class="text-center p-4 table-item-loading" hidden>
...
<script type="text/javascript">
  ...
  function deleteBaris(tableID, objRow=null) {
    ...
    try {
      setItemOrderAdditional($(`#${tableID} > tbody > tr`).length, tableID);
    } catch(error){}
  ...
  $('.btn-submit').on('click', function(e){
    try {
      ...
      if(!form_valid){
        Swal.fire('Add New Template', 'Please make sure all data required is filled or selected', 'warning');
        return false;
      }
    } catch(error){}
    ...
    if(data_item.length < 1){
      ...
      $.ajax({
        ...
        type: "GET",
        dataType: 'json',
        data: {'type': 'duplicate-template', 'plant': plant, 'template_name': template_name},
