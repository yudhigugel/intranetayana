...
@section('title', 'Group Role (Assign Menu) - Edit')
                ...
                <fieldset class="form-group">
                  ...
                  @if(isset($data_menu_map))
                    ...
                    @foreach($data_menu_map as $key => $parent)
                      <div class="col-lg-3 mb-3 wrapper-menu" data-mapping-key="{{ $key }}">
                        <h6 class="mb-2"><input type="checkbox" name="menu[]" value="{{ $parent['SEQ_ID'] }}" @if(isset($assigned_rolemenu) && in_array($parent['SEQ_ID'], $assigned_rolemenu)) checked @endif class="mr-2 checkbox-title" data-mapping-key="{{ $key }}" data-unique="{{ $parent['SEQ_ID'] }}">{{ $parent['MENU_NAME'] }}</h6>
                        @if(isset($parent['child']) && count($parent['child']))
                          @foreach($parent['child'] as $key_pair => $child_1)
                            <div class="mb-1 offset-1 pl-2">
                              <a class="text-secondary d-block mb-1">
                                <input type="checkbox" name="menu[]" value="{{ $child_1['SEQ_ID'] }}" @if(isset($assigned_rolemenu) && in_array($child_1['SEQ_ID'], $assigned_rolemenu)) checked @endif class="mr-1 checkbox-parent" data-mapping-key="{{ $key }}" data-unique="{{ $child_1['SEQ_ID'] }}" data-parent-id="{{ $child_1['PARENT_ID'] }}">
                            ...
                            @if(isset($child_1['child']) && count($child_1['child']))
                              @foreach($child_1['child'] as $key_pair => $child_2)
                                <div class="collapse show offset-2 ml-3 pl-2" id="icons-{{ $child_2['PARENT_ID'] }}">
                                  <ul class="nav flex-column sub-menu">
                                    <li><input type="checkbox" name="menu[]" value="{{ $child_2['SEQ_ID'] }}" @if(isset($assigned_rolemenu) && in_array($child_2['SEQ_ID'], $assigned_rolemenu)) checked @endif class="mr-1 checkbox-child" data-mapping-key="{{ $key }}" data-parent-id="{{ $child_2['PARENT_ID'] }}"><small class="menu-title" style="font-size: 90%">{{ $child_2['MENU_NAME'] }}</small></li>
                                  </ul>
                                </div>
                              @endforeach
                            @endif
